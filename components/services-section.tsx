    ...
    <section className="bg-white py-12 md:py-16 px-4 md:px-8">
      <div className="container max-w-6xl mx-auto">
        <div className="text-center mb-8 md:mb-12">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-primary mb-3 md:mb-4 px-4">
            How We Support Your Success
          </h2>
          <p className="text-base md:text-lg text-foreground max-w-2xl mx-auto px-4">
          ...
          {services.map((service, index) => (
            <Card key={index} className="border-none shadow-lg hover:shadow-xl transition-shadow">
              <CardContent className="p-5 md:p-6 text-center">
                <div className="inline-flex items-center justify-center w-14 h-14 md:w-16 md:h-16 mb-3 md:mb-4 rounded-full bg-primary text-white">
                  <service.icon className="w-7 h-7 md:w-8 md:h-8" />
                </div>
                <h3 className="text-lg md:text-xl font-bold text-primary mb-2">{service.title}</h3>
                <p className="text-sm md:text-base text-foreground">{service.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
